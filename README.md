...
- Сравнение до четырёх производителей бок о бок по стране, году основания, размеру модельного ряда, новейшей модели и среднему году выпуска: лучшие значения подсвечиваются
...
![Сравнение производителей](docs/screenshots/05-compare.png)
...
**API** (`/api/AutoMarket/AutoMarket`). Строка подключения задаётся в `appsettings.Development.json` (создайте сами, в репозитории её нет):
...
Клиент поднимается на `http://localhost:4200`, API-адрес задан в `src/environments/environment.ts`.
...
- Страницы: производители (карточки), модели (таблица), сравнение, о проекте; формы добавления и редактирования вынесены в отдельные маршруты
- Выбор для сравнения хранится в `CatalogStore` и дублируется в `localStorage`, поэтому переживает перезагрузку страницы
